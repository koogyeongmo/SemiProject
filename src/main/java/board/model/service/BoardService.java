...
//		-- 끝페이지 endPageNum =  (endPageNum > 전체페이지수) ? 전체페이지수 : startPageNum+페이지수 - 1;
	...
	public Map<String, Object> selectPageListDetail(int pageSize, int pageBlockSize, int currentPageNum) {
		Map<String, Object> result = null;
		...
		List<BoardDto> dtolist = dao.selectPageListDetail(session, start, end);
		session.close();
