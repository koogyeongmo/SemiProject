...
<script>
	...
	// Create a map to store colors for each unique text
	const textToColorMap = {};
	
	board_flairs.forEach((flair) => {
	    const text = flair.textContent.trim(); // Get the trimmed text content
	
	    // If the text is not in the map, assign a color from the colors array
	    ...
	    // Set the background color of the flair using the mapped color
